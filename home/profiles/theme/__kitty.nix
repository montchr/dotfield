...
lib.mkIf cfg.enable (
  ...
  in
  {
    ...
    xdg.configFile = {
      "kitty/theme-dark.conf".text = makeConf (makeThemeAttrs (asHexStrings colorSchemes.dark.colors));
      "kitty/theme-light.conf".text = makeConf (makeThemeAttrs (asHexStrings colorSchemes.light.colors));
    };
  }
)

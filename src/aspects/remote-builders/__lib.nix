flake@{ ... }:
{
  dotfield.aspects.defaults.nixos =
    { config, ... }:
    {
      ...
      lib.dotfield.mkBuildMachineFor =
        ...
        let
          localVcpus = flake.config.dotfield.meta.hosts.${localHostName}.hardware.vcpus or 1;
          builder = flake.config.dotfield.meta.hosts.${builderHostName}.hardware or { };
